...
def dessiner_ligne(x1: int, y1: int, x2: int, y2: int, canvas: Canvas) -> None:
    ...
    arguments:
        ...
        canvas (Canvas):  canevas dans lequel nous voulons travailler
    """
...
def tracer_rectangle(x1: int, y1: int, x2: int, y2: int, canvas: Canvas) -> int:
    ...
    """
    return canvas.create_rectangle(x1, y1, x2, y2, fill="black")
...
def trace_formes(
    ...
    for y in range(nb_ligne):
        ...
        for x in range(nb_colonne):
            ...
            x2 = x1 + (taille / nb_colonne) - 2
            y2 = y1 + (taille / nb_ligne) - 2
...
def mise_a_jour_couleur_formes(
    ...
    arguments:
        ...
        can (Canvas): canevas dans lequelle nous travaillons
    """
